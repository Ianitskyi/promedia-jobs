...
**Status: not yet executed.** Everything in `docs/ARCHITECTURE_V2.md` and
this app's Vitest suite (`npm test`) describes and unit-tests the
*intended* behavior of the schema and RPC functions in
`supabase/migrations/0001_init.sql` and
`supabase/migrations/0002_platform_refactor.sql` against a **mocked**
Supabase client — none of it has been run against a real Postgres
instance. Do not treat the database security/integrity model as
verified — only as designed and reasoned about — until every item below
has actually been run against a live (or local, via `supabase start`)
project and passed. Check items off (`[x]`) as you confirm them, and
note the project/date.
